...
values 
	...
	-- ('HD_GREATPERSON_BUILDING_JNR_TOOLING_SHOP_PRODUCTION',		'Summary',	'LOC_HD_GREATPERSON_BUILDING_JNR_TOOLING_SHOP_PRODUCTION');

insert or replace into ModifierStrings
	(ModifierId,		Context,	Text)
select ModifierId, 'Summary','LOC_HD_GREATPERSON_BUILDING_JNR_TOOLING_SHOP_PRODUCTION' from Modifiers where ModifierId = 'HD_GREATPERSON_BUILDING_JNR_TOOLING_SHOP_PRODUCTION';
...
insert or replace into GreatPersonIndividualActionModifiers
	(GreatPersonIndividualType,						ModifierId,												AttachmentTargetType)
select
	'GREAT_PERSON_INDIVIDUAL_GUO_SHOU_JING',		'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'GREAT_PERSON_ACTION_ATTACHMENT_TARGET_DISTRICT_IN_TILE'
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS';

insert or replace into Modifiers
	(ModifierId,											ModifierType,											SubjectRequirementSetId)
select
	'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'MODIFIER_PLAYER_CITIES_TERRAIN_ADJACENCY',				NULL
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS';

insert or replace into ModifierArguments
	(ModifierId,											Name,					Value)
select
	'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'Amount',				1
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS' union all
select
	'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'Description',			'LOC_DISTRICT_' || TerrainClassType || '_SCIENCE'
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS' union all
select
	'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'DistrictType',			'DISTRICT_CAMPUS'
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS' union all
select
	'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'TerrainType',			TerrainType
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS' union all
select
	'GREAT_PERSON_HD_' || TerrainType || '_EXTRA_BOUNS',	'YieldType',			'YIELD_SCIENCE'
from TerrainClass_Terrains where TerrainClassType = 'TERRAIN_CLASS_HILLS';
